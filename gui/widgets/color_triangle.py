'''
COLOR SELECTOR.
ALL CREDITS GOES TO: https://github.com/iLLiCiTiT/PyQtColorTriangle

THE ORIGINAL CODE WAS MODIFIED IN ORDER TO SUIT THE PURPOSE OF THIS PROJECT.
'''
...
class QtColorTriangle(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.bg_image = QImage(
            self.sizeHint(), QImage.Format_RGB32
        )
        self.cur_color = QColor()
        self.point_a = QPointF()
        self.point_b = QPointF()
        self.point_c = QPointF()
        self.point_d = QPointF()
        ...
        self.selector_pos = QPointF()

        self.sel_mode = TriangleState.IdleState

        self._triangle_outline_pen = QPen(
            QColor(40, 40, 40, 128),
            2
        )
    ...
    def paintEvent(self, event):
        ...
        pix_painter.setPen(QPen(Qt.white, self.pen_width))
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.sel_mode is TriangleState.SelectingHueState:
            ...
            aa = Vertex(Qt.transparent, self.point_a)
            bb = Vertex(Qt.transparent, self.point_b)
            cc = Vertex(Qt.transparent, self.point_c)
    ...
    def drawTrigon(self, buf, pa, pb, pc, color):
        ...
        p1 = Vertex(color, pa)
        p2 = Vertex(Qt.black, pb)
        p3 = Vertex(Qt.white, pc)
        ...
        for y in range(p1yfloor, p3yfloor):
            ...
            if xdist != 0.0:
                ...
                for x in range(lxi, rxi + 1):
                    buf.setPixel(x, y, qRgb(int(r), int(g), int(b)))
                    r += rdelta
                    g += gdelta
                    b += bdelta
    ...
    def paint_bg(self):
        bg_image = QPixmap(self.contentsRect().size())
        ...
        hue_gradient = QConicalGradient(
            bg_image.rect().center(), 90 - self.hue_offset
        )
        sat_val_gradient = QConicalGradient(
            bg_image.rect().center(), 90 - self.hue_offset
        )

        hue_color = QColor()
        sat_val_color = QColor()
        _, sat, val, _ = self.cur_color.getHsv()
    ...
    def _move_point_to_triangle(self, x, y, a, b, c):
        ...
        angleP = self._angle_at(QPointF(x, y), self.contentsRect())
